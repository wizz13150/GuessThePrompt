...
import type { Difficulty, GameMode } from './services/types.js';
...
function readDistFile(relativePath: string) {
  ...
  if (!filePath.startsWith(distDir) || !existsSync(filePath)) return null;
...
function serveIndex(c: any) {
  ...
  if (!file) return c.text('Production build missing. Run npm run build.', 500);
...
function hasCivitaiApiKey(c?: any) {
...
app.post('/api/game/new-round', async (c) => {
  const body = await c.req.json().catch(() => ({}));
  const mode = (body.mode === 'classic' ? 'classic' : 'adventure') as GameMode;
  const difficulty = (['easy', 'normal', 'hard'].includes(body.difficulty) ? body.difficulty : 'normal') as Difficulty;
...
app.get('/assets/*', (c) => {
  ...
  if (!file) return c.text('Asset not found', 404);
...
app.get('/manifest.webmanifest', (c) => {
  ...
  if (!file) return c.text('Manifest not found', 404);
