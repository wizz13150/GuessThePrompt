...
import { DEFAULT_GENERATION_SETTINGS, DEFAULT_MODEL_AIR, MODEL_PRESETS, type GenerationLora, type GenerationSettings } from './types.js';
...
function presetFor(modelPreset: GenerationSettings['modelPreset']) {
  return MODEL_PRESETS.find((preset) => preset.id === modelPreset);
}

function isSdxlPreset(modelPreset: GenerationSettings['modelPreset'], modelAir: string): boolean {
  return modelPreset === 'sdxl-base' || modelAir.startsWith('urn:air:sdxl:');
}

function isKleinPreset(modelPreset: GenerationSettings['modelPreset'], modelAir: string): boolean {
  return modelPreset === 'flux2-klein-9b' || modelAir.startsWith('urn:air:flux2:');
}

function isZImagePreset(modelPreset: GenerationSettings['modelPreset'], modelAir: string): boolean {
...
function clampNumber(value: unknown, fallback: number, min: number, max: number, integer = true): number {
...
export class GenerationSettingsStore {
  ...
  read(): GenerationSettings {
    if (!existsSync(SETTINGS_PATH)) return { ...DEFAULT_GENERATION_SETTINGS, loras: [] };
    try {
      const parsed = JSON.parse(readFileSync(SETTINGS_PATH, 'utf8')) as Partial<GenerationSettings>;
  ...
  update(patch: Partial<GenerationSettings>): GenerationSettings {
  ...
  private normalize(input: Partial<GenerationSettings>): GenerationSettings {
    const requestedPreset = String(input.modelPreset || '');
    const modelPreset = ALLOWED_PRESETS.has(requestedPreset as GenerationSettings['modelPreset'])
      ? requestedPreset as GenerationSettings['modelPreset']
