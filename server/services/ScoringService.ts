import type { Difficulty, RoundState, ScoreBreakdown } from './types.js';

const DIFFICULTY_RULES: Record<Difficulty, { label: string; multiplier: number; timeWindowSeconds: number; maxTimeBonus: number; attemptPenalty: number }> = {
...
function gradeFor(total: number): ScoreBreakdown['grade'] {
  ...
  if (total >= 1450) return 'A';
  if (total >= 1100) return 'B';
  if (total >= 750) return 'C';
  return 'D';
}

export function getScoreRules(difficulty: Difficulty) {
  return DIFFICULTY_RULES[difficulty];
}

export function calculateRoundScore(round: RoundState, accuracy: number, streakBeforeRound: number): ScoreBreakdown {
