...
function isUnsafeNsfw(value: unknown): boolean {
  if (value === true) return true;
  if (typeof value === 'string') return ['true', 'soft', 'mature', 'x'].includes(value.toLowerCase());
  return false;
}

function splitPromptTokens(prompt: string): string[] {
  return prompt
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
...
function deriveAnswer(prompt: string, fallbackId: string): string {
...
function titleFromAnswer(answer: string, prompt: string): string {
...
function scoreImage(item: CivitaiImageItem, prompt: string): number {
...
function tagsFromPrompt(prompt: string, answer: string): string[] {
...
export class CivitaiPromptImporter {
  constructor(private readonly apiKey = (process.env.CIVITAI_API_KEY || '').trim()) {}

  async importFromImages(limit = 300): Promise<ImportResult> {
    const target = Math.max(50, Math.min(limit, 800));
    const pageSize = 100;
    const items: CivitaiImageItem[] = [];
    let cursor = '';
    ...
    while (items.length < target) {
      ...
      const payload = (await response.json()) as { items?: CivitaiImageItem[]; metadata?: { nextCursor?: string } };
