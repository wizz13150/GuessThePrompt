...
function promptFragment(prompt, answer) {
...
export function getHintAvailability(round) {
    if (round.solved)
        return { nextLevel: null, available: false, requiredAttempts: null, cost: null, reason: 'Round already solved.' };
    if (round.gaveUp)
        return { nextLevel: null, available: false, requiredAttempts: null, cost: null, reason: 'Round abandoned.' };
    const level = nextHintLevel(round);
    if (!level)
        return { nextLevel: null, available: false, requiredAttempts: null, cost: null, reason: 'All hints unlocked.' };
...
export function unlockNextHint(round) {
