...
function readImportedCache(path) {
    try {
        if (!existsSync(path))
            return [];
...
function normalizeRecord(record) {
    if (record.nsfw || !record.prompt || !isPromptSafe(record.prompt))
        return null;
    return {
        ...record,
...
function uniqueById(records) {
    ...
    for (const record of records) {
        ...
        if (!existing || record.qualityScore > existing.qualityScore)
...
export class PromptCacheStore {
    cachePath;
    prompts;
    constructor(seedRecords = PROMPT_SAMPLES, cachePath = DEFAULT_CACHE_PATH) {
